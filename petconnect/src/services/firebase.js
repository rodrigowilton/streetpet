...
export async function registerUser(data) {
  ...
  const user = {
    ...
    totalPets: 0, fcmToken: ''
  };
  await setDoc(doc(db, 'pc_users', uid), user);
  await addDoc(collection(db, 'pc_notifications'), {
    userId: uid,
    title: '🐾 Bem-vindo ao PetConnect!',
    message: 'Seu acesso é válido por 30 dias. Cadastre seus pets e explore a comunidade!',
...
export async function loginUser(email, password) {
  ...
  if (!user.isAdmin) {
    ...
    if (expiry && expiry < now) throw new Error('EXPIRED');
  }
  return user;
}
...
export { onAuthStateChanged, auth as authInstance };

// ===================== PETS =====================
export async function getPets(userId) {
  const q = query(collection(db, 'pc_pets'), where('ownerUid', '==', userId), orderBy('createdAt', 'desc'));
...
export async function updatePet(petId, data) {
  await updateDoc(doc(db, 'pc_pets', petId), { ...data, updatedAt: serverTimestamp() });
}
...
export async function getVaccines(petId) {
  const q = query(collection(db, 'pc_vaccines'), where('petId', '==', petId), orderBy('date', 'desc'));
  const snap = await getDocs(q);
  return snap.docs.map(d => ({ id: d.id, ...d.data() }));
}
...
export async function getConsults(petId) {
  const q = query(collection(db, 'pc_consults'), where('petId', '==', petId), orderBy('date', 'desc'));
  const snap = await getDocs(q);
  return snap.docs.map(d => ({ id: d.id, ...d.data() }));
}
...
export async function getWeights(petId) {
  const q = query(collection(db, 'pc_weights'), where('petId', '==', petId), orderBy('date', 'desc'), limit(20));
  const snap = await getDocs(q);
  return snap.docs.map(d => ({ id: d.id, ...d.data() }));
}
...
export async function getPosts(lim = 30) {
  const q = query(collection(db, 'pc_posts'), orderBy('createdAt', 'desc'), limit(lim));
  const snap = await getDocs(q);
  return snap.docs.map(d => ({ id: d.id, ...d.data() }));
}
...
// ===================== PERDIDOS & ADOÇÃO =====================
export async function getLostPets(type = 'all') {
  let q;
  if (type === 'all') q = query(collection(db, 'pc_lost'), orderBy('createdAt', 'desc'), limit(30));
  else q = query(collection(db, 'pc_lost'), where('type', '==', type), orderBy('createdAt', 'desc'), limit(30));
  const snap = await getDocs(q);
  return snap.docs.map(d => ({ id: d.id, ...d.data() }));
}
...
export async function getAdminStats() {
  ...
  return {
    ...
    expiring: users.filter(u => {
      ...
      return diff >= 0 && diff <= 7 && !u.isExpired;
    }).length,
    expired: users.filter(u => u.isExpired || (u.expirationDate?.toDate?.() < new Date())).length
  };
}
